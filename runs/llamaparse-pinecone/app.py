...
system_prompt_template = "You are a helpful AI assistant specialized in biochemistry, cell biology and biomanufacturing. Use the following pieces of context to answer the human's question. If you don't know the answer, just say that you can't find the answer from the context, don't try to make up an answer. Context: {context}"
uploaded_files = []

# Parse command-line arguments
parser = argparse.ArgumentParser(description="CytoRAG : Cell Biology with RAG")
...
# Gradio interface
with gr.Blocks(css=css, fill_height=True, title="CytoRAG: Cell Biology with Rag") as demo:
    gr.Markdown("# CytoRAG: Cell Biology Copilot with LlamaIndex, Pinecone, Arize Phoenix on Vessl")

    gr.Markdown("""
    Parse PDF documents from 10 millions literature on PubMed of NIH. https://pubmed.ncbi.nlm.nih.gov/" 
    """)
